...
(defn- coord->version
  "Returns Maven-representable version string, or nil (skip).
   ...
   - {:local/root \"./...\" or \"../...\"} -> local-root-version ONLY when:
       ...
       * artifactId(lib) == artifactId(:name)
     Otherwise relative local/root: nil (skipped).
  ...
  (cond
    ...
    (let [root (str (:local/root coord))
      ...
      (cond
        (and rel? (some? name))
        (let [[_ a-lib]  (lib->ga lib)
              [_ a-name] (lib->ga name)]
          (when (= a-lib a-name)
...
(defn- dependency-node
  [{:keys [groupId artifactId version scope optional]}]
  {:tag     :dependency
   :attrs   nil
...
(defn splice-dependencies-section
  "Pure function: returns updated pom.xml text with <dependencies> replaced or inserted.
   ...
   - If not present: insert before closing </project>.
...
(defn- splice-dependencies-section!
  ...
  [pom-path deps-xml]
  (let [old (slurp pom-path :encoding "UTF-8")
        new (splice-dependencies-section old deps-xml)]
    (when-not (= old new)
      (spit pom-path new :encoding "UTF-8"))
...
(defn sync-pom-deps!
  "Reads deps.edn :deps, renders <dependencies> XML, and splices it into pom.xml.
   ...
   Options:
     :name (lib symbol, used only for relative :local/root replacement rule)
     :local-root-version (default \"${project.version}\")"
